//Taọ obj chứa nội dung cho manufacturer
var dataList = {
        'Apple': ['iPhone', 'iPad', 'Macbook'],
        'Sam Sung': [' Samsung galaxy 5', 'Samsung galaxy 5 s '],
        'LG': ['LG01", "LG02']
    }
    //gọi đến obj có id của tag
var manufacturerTag = document.getElementById('manufacturer_name')

for (m_name in dataList) { //để láy key trong obj đưa vào option tag
    console.log(m_name)
    manufacturerTag.innerHTML += `<option value="${m_name}">${m_name}</option>`
}
//Xử lý đưa thông tin vào category obj
//gọi obj vào biến categoryTag
var categoryTag = document.getElementById('category_name')
    //dung fuction để láy list value theo key đã chọn ở trên
function changeManufacturerName() {
    var value = manufacturerTag.value //đặt obj chứa list key
        //tạo option đầu tiên có tên select
    categoryTag.innerHTML = `<option value="">--Select--</option>`
    if (categoryTag != "") {
        var categoryList = dataList[value] //ddặt biến chứa list value theo key được chứa trong value
